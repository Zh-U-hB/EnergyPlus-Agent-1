...
> **图 1　EnergyPlus\-Agent 总体架构。** 系统以分层方式自上而下组织：设计者经自然语言文本与建筑图纸输入需求，输入解析模块将其转化为结构化建筑意图；在生成专业物理参数前，RAG 控制模块检索 EnergyPlus 参考库（材料/构造/日程/设计日）以约束参数幻觉；解析模块在 MCP 工具层与中心配置状态上经多阶段 agent 编排逐阶段构造可校验对象，并由多层正确性保障把关；通过验证的模型生成 IDF 并交由 EnergyPlus 仿真；结果解析模块把输出转化为设计者可读的指标（能耗/EUI/舒适/峰值/3D 辐照），虚线反馈回路驱动下一轮设计迭代。
...
flowchart TB
    ...
    %% ============ L1 用户输入层 ============
    subgraph L1 ["用户输入层  User Input"]
        TXT["自然语言建筑描述<br/>Natural-language description"]
        IMG["建筑图纸<br/>（平面/立面/剖面/轴测）<br/>Architectural drawings"]
        EPW["气象文件 EPW<br/>Weather file"]
    end

    %% ============ L2 输入解析层 ============
    subgraph L2 ["输入解析层  Intent Parsing"]
        INT["多模态大语言模型<br/>Multimodal LLM"]
        IO["结构化建筑意图<br/>Structured output (IntakeOutput)"]
        INT -->|"结构化输出"| IO
    end

    %% ============ L3 RAG 知识层 ============
    subgraph L3 ["RAG 知识层  Retrieval-Augmented Control"]
        REF[("EnergyPlus 参考库<br/>材料 / 构造 / 日程 / 设计日<br/>Materials · Constructions · Schedules · Design days")]
        SRCH["向量检索 Vector retrieval<br/>（Qdrant + Gemini Embedding）"]
        REF --> SRCH
    end

    %% ============ L4 智能体编排层 ============
    subgraph L4 ["智能体编排层  Agent Orchestration"]
        direction LR
        WF["多阶段 Agent 编排<br/>LangGraph 多节点工作流"]
        MCP["MCP 工具层<br/>结构化 CRUD（Building / Zone /<br/>Material / Construction / Surface /<br/>Fenestration / Schedule / HVAC / People / Lights）"]
        CS[("中心配置状态<br/>ConfigState")]
        VAL["多层正确性保障<br/>Pydantic Schema + 跨引用校验<br/>+ 自修复 + 人机审查"]
        WF -->|"调用工具"| MCP
        MCP -->|"读写对象"| CS
        CS -->|"校验"| VAL
    end
    ...
    %% ============ L5 仿真与结果层 ============
    subgraph L5 ["仿真与结果层  Simulation & Results"]
        IDF["IDF 生成"]
        EP["EnergyPlus 仿真引擎"]
        ANZ["结果解析与可视化<br/>能耗 / EUI / 热舒适 / 峰值负荷<br/>3D 热区能耗 / 外表面太阳辐照"]
        IDF --> EP --> ANZ
    end
    ...
    IMG --> INT
    EPW --> IDF
    IO -->|"专业参数<br/>取值依据"| SRCH
    SRCH -->|"检索记录"| WF
    VAL -->|"可运行模型"| IDF

    %% ============ 反馈回路（虚线） ============
    ANZ -.->|"设计反馈 Design feedback<br/>（驱动下一轮迭代）"| DES
    VAL -.->|"校验错误<br/>回灌自修复"| WF

    %% ============ 分层着色（浅色背板，对齐论文风格） ============
    style L1 fill:#f5f7fa,stroke:#9aa5b1,stroke-width:1px
    style L2 fill:#eef4fb,stroke:#4a7fb8,stroke-width:1.5px
    style L3 fill:#eef7f0,stroke:#4a8a5e,stroke-width:1.5px
    style L4 fill:#fdf4ee,stroke:#c87a3b,stroke-width:1.5px
    style L5 fill:#f4eefb,stroke:#7a4fb6,stroke-width:1.5px
    style DES fill:#ffffff,stroke:#2b2b2b,stroke-width:2px
```

> 【图例】实线箭头表示主数据流（输入 → 解析 → 检索 → 编排 → 仿真 → 解析），虚线箭头表示反馈回路（结果反馈驱动设计者下一轮迭代、校验错误回灌 agent 自修复）。分层背板颜色仅用于区分模块，无其他含义。L1–L5 与正文 3\.1–3\.3 的模块对应：L2 为输入解析模块、L3 为 RAG 控制模块、L4 为解析模块、L5 为仿真执行与结果解析。
