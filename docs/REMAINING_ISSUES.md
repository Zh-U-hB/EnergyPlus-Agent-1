...
> **状态**：待实现。
> **关联**：`tests/agent/DIAGNOSTICS.md`（根因分析）、`docs/BACKHOP_REDESIGN.md`（回跳机制目标框架）、`docs/BACKHOP_IMPLEMENTATION_SPEC.md`（回跳重实现规格）。
...
**诊断根因**：C（`tests/agent/DIAGNOSTICS.md`）
...
**诊断根因**：D（`tests/agent/DIAGNOSTICS.md`）
...
```
- `produced_output`（`_check_simulation_output`）：只要存在 `eplusout.end`/`.sql`/`eplustbl.csv` 任一即 True（空建筑也会生成）
...
**修复方向**：
在 `_check_simulation_output` 或 `simulation_ok` 的判定（均位于 `tests/agent/test_run_robustness.py`）加门槛——要求仿真用的 IDF 至少含 1 个 Zone（或解析 `eplusout.eio` 的 `Zone Summary` 行，要求 Number of Zones > 0）：
```python
...
**诊断根因**：B（`tests/agent/DIAGNOSTICS.md`）
...
**诊断根因**：A 的源头（`tests/agent/DIAGNOSTICS.md`）
